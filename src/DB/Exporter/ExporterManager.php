...
use Drc\DB\Exporter\DriverExporter\AbstractDriverExporter;
...
class ExporterManager
{
    ...
    public function migrate($database = null,$custom = null)
    {
        if(is_null($custom))
            ...
            if(isset($this->app['config']['drc.exportConfig.'.$custom.'.migration']))
            {
                if($exporter->isEmptyStorePath())
                    $exporter = $exporter->setStorePath($this->app['config']['drc.exportConfig.' . $custom.'.migration']);
            }
    ...
    public function seed($database = null,$custom = null)
    {
        if(is_null($custom))
            ...
            if(isset($this->app['config']['drc.exportConfig.'.$custom.'.seed'])) {
                if($exporter->isEmptyStorePath())
                    $exporter = $exporter->setStorePath($this->app['config']['drc.exportConfig.' . $custom.'.seed']);
            }
            $exporter->convert($database)->write();
