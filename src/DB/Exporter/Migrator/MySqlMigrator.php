...
class MySqlMigrator extends MySqlAction
{
    ...
    public function write()
    {
        ...
        $this->filePath = config('drc.exportConfig.migrations')."{$filename}";

        if(!File::exists(config('drc.exportConfig.migrations'))){
            File::makeDirectory(config('drc.exportConfig.migrations'));
        }
    ...
    protected function createStubMigration($table) {
        ...
        if (is_array($tableIndexes) && count($tableIndexes)){
